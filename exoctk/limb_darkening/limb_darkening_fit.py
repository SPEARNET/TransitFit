...
def ld_profile(name='quadratic', latex=False):
    ...
    if name in names:
        ...
        if latex:
            ...
            for i, j in [('**', '^'), ('m', r'\mu'), (' ', ''), ('np.', '\\'), ('0.5', '{0.5}'), ('1.5', '{1.5}')]:
...
class LDC:
    ...
    """
    def __init__(self, model_grid='ACES', interp=True):
    ...
    def calculate(self, Teff, logg, FeH, profile, mu_min=0.05, ld_min=0.01,
                  bandpass=None, name=None, color=None, interp=True, **kwargs):
        """
        ...
        grid_point = self.get_model(Teff, logg, FeH, interp=self.interp)
        if not self.interp:
    ...
    def get_model(self, teff, logg, feh, interp=True):
        """
        ...
        if params in self.model_cache:
            ...
            #breakpoint()
            model = self.model_grid.get(teff, logg, feh, interp=self.interp)
